...
1. `start_sdlc_workflow` – kicks off the workflow and returns the `workflow_id` / `run_id` so the client can poll later. Required params: `issue_url`, `repository_path`. Optional override: `branch_name`.
