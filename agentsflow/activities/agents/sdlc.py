...
def _build_agent(*, name: str, instructions: str, output_type):
    _ensure_openai_key()
...
def _implementation_agent() -> Agent:
    return _build_agent(
...
@lru_cache(maxsize=None)
def _evaluation_agent() -> Agent:
    return _build_agent(
        name="sdlc-evaluation",
        instructions=(
            "You evaluate coding transcripts for tracked issues. Review the issue details alongside the transcript and decide whether the implementation sounds complete and whether automated tests were carried out. "
...
@lru_cache(maxsize=None)
def _tests_agent() -> Agent:
    return _build_agent(
        name="sdlc-tests",
        instructions=(
            "You design automated test coverage for the tracked issue. Focus on the uncovered gaps highlighted by the evaluation. "
            "Provide a short summary, enumerate concrete test cases, and list any tooling commands needed to execute them."
        ),
...
@lru_cache(maxsize=None)
def _release_agent() -> Agent:
    return _build_agent(
        name="sdlc-release",
        instructions=(
            "Draft the source control release plan for the task. Provide an actionable branch name, commit message, and pull request summary. "
            "Include follow-up items if reviewers flagged any. Do not include markdown formatting characters such as backticks."
        ),
