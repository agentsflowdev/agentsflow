...
- A Temporal cluster (the dev server from `temporal server start-dev` works)
...
- Or reuse the compose service: `docker compose up temporal`

## 4) Run the worker
```bash
uv run python -m agentsflow.worker --task-queue agentsflow-sdlc
```
The worker loads credentials from `.env`, registers the Git, issue, and ACP activities, and listens until interrupted.
