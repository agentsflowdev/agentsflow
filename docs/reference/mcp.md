...
- `await_agentsflow_result(workflow_id)` – Waits for the latest run of the workflow and returns the serialized `ProcessWorkflowOutput`. If the run paused for clarification, the payload contains the open questions/assumptions instead.
